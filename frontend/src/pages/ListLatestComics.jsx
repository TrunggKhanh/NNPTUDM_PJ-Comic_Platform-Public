...
import { fetchLatestComics } from "../services/BoTruyenServices";
import { Link } from "react-router-dom";

import Loader from "../components/Element/Loader";

const ListLatestComics = () => {
  ...
  const loadComics = async (page) => {
    setLoading(true);
    try {
      const { comics: fetchedComics, totalPages } = await fetchLatestComics(page);
              ...
              <div key={comic._id} className="item col-2 update-item">
                <Link to={`/comic/${comic._id}`}>
                  <figure className="position-relative">
                    {comic.AnhBia && (
                      <>
                        <span className="hot-icon">NEW</span>
                        <img
                          loading="lazy"
...
export default ListLatestComics;
